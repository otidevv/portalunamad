...
<div class="p-6">
    ...
    <div class="bg-white rounded-lg shadow-sm overflow-hidden">
        <div class="overflow-x-auto">
            <table class="w-full">
                    <caption class="sr-only">Listado de categorías de comunicados</caption>
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                    {{ $categoria->comunicados_count }} comunicados
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex items-center space-x-2">
                                    <a href="{{ route('admin.comunicado-categorias.show', $categoria) }}" 
                                       class="text-gray-600 hover:text-gray-900" title="Ver" aria-label="Ver categoría {{ $categoria->nombre }}">
                                        <svg aria-hidden="true" focusable="false" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <a href="{{ route('admin.comunicado-categorias.edit', $categoria) }}" 
                                       class="text-blue-600 hover:text-blue-900" title="Editar" aria-label="Editar categoría {{ $categoria->nombre }}">
                                        <svg aria-hidden="true" focusable="false" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" 
                                    ...
                                    @if($categoria->comunicados_count == 0)
                                        <form action="{{ route('admin.comunicado-categorias.destroy', $categoria) }}" 
                                              method="POST" 
                                              class="inline-block"
                                              onsubmit="return confirm('¿Está seguro de eliminar esta categoría?');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="text-red-600 hover:text-red-900" title="Eliminar" aria-label="Eliminar categoría {{ $categoria->nombre }}">
                                                <svg aria-hidden="true" focusable="false" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" 
...
@push('scripts')
<script>
function toggleEstado(categoriaId, estadoActual) {
    fetch(`/admin/comunicado-categorias/${categoriaId}/estado`, {
        method: 'POST',
        headers: {

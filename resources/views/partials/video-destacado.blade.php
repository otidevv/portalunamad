    ...
    <div id="video-destacado-modal"
        ...
        <div id="video-destacado-tarjeta"
            ...
            <button type="button"
                    ...
                    class="absolute top-4 right-4 z-10 flex items-center justify-center w-10 h-10 rounded-full bg-black/50 text-white hover:bg-black/70 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-white transition-colors">
            ...
            <div class="px-6 py-6 sm:px-8 sm:py-7 border-t-4 border-[#db0455]">
                ...
                @if($videoDestacado->descripcion)
                    <p id="video-destacado-descripcion" class="text-sm sm:text-base text-gray-600 leading-relaxed mt-2.5">{{ $videoDestacado->descripcion }}</p>
                @endif
                <p class="text-xs text-gray-600 mt-3">Puede cerrar esta ventana con la tecla Escape. @if($videoDestacado->tipo !== 'archivo')Los subtítulos se activan desde el botón «CC» del reproductor.@endif</p>

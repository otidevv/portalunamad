...
class NoticiasGobPeService
{
    private const FUENTE_URL = 'https://www.gob.pe/institucion/unamad/noticias';

    private const BASE_URL = 'https://www.gob.pe';

    private const CACHE_KEY = 'noticias_gob_pe_unamad';
    ...
    private function fetch(): array
    {
        try {
            ...
            if (! $response->ok()) {
                Log::warning('gob.pe scraping HTTP no OK', ['status' => $response->status()]);
            ...
            Log::error('Error scraping gob.pe noticias', ['error' => $e->getMessage()]);
    ...
    private function parse(string $html): array
    {
        ...
        $items = $xpath->query(
            "//li[.//a[contains(@href, '/noticias/')]] | //div[contains(concat(' ', normalize-space(@class), ' '), ' card ') and .//a[contains(@href, '/noticias/')]]"
        );
        ...
        foreach ($items as $item) {
            $links = $xpath->query(".//a[contains(@href, '/noticias/')]", $item);
            if ($links->length === 0) {
                continue;
            }
        ...
        usort($noticias, fn ($a, $b) => strcmp($b['fecha_iso'] ?? '', $a['fecha_iso'] ?? ''));
